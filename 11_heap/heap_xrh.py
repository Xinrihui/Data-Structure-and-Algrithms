...
class Heap:
    ...
    def __up_to_down_max_heapify(self, p):
        ...
        while p < self.length:
            ...
            self.heap_list[max_index], self.heap_list[p] = self.heap_list[p], self.heap_list[
                max_index]  # 当前节点与 最大节点进行交换
    ...
    def __down_to_up_max_heapify(self, p):
        ...
        while p > 0:
            ...
            self.heap_list[p], self.heap_list[min_index] = self.heap_list[min_index], self.heap_list[
                p]  # 当前节点和 最小节点 进行交换
    ...
    def __up_to_down_min_heapify(self, p):
        ...
        while p < self.length:
            ...
            self.heap_list[min_index], self.heap_list[p] = self.heap_list[p], self.heap_list[
                min_index]  # 当前节点与 最小节点进行交换
    ...
    def __down_to_up_min_heapify(self, p):
        ...
        while p > 0:
            ...
            self.heap_list[p], self.heap_list[max_index] = self.heap_list[max_index], self.heap_list[
                p]  # 当前节点和 最小节点 进行交换
...
class HeapTuple:
    ...
    def __init__(self, input_list, compare_func=lambda x: x, flag='large'):  # 默认为 构建大顶堆
        """

        :param input_list: [ (key1,value1), ... ]
        :param compare_func: 指定 比较键 的 lambda 函数，堆 根据此键 来比较元素之间的大小
        :param flag:
            flag=='large' 构建 大顶堆
            flag=='small' 构建 小顶堆
    ...
    def __up_to_down_max_heapify(self, p):
        ...
        while p < self.length:
            ...
            # 找出 左子节点 和 右子节点  中的最大节点
            if left_child < self.length and self.compare_func(self.heap_list[left_child]) > self.compare_func(self.heap_list[max_index]):
                max_index = left_child

            if right_child < self.length and self.compare_func(self.heap_list[right_child]) > self.compare_func(self.heap_list[max_index]):
                max_index = right_child
    ...
    def __down_to_up_max_heapify(self, p):
        ...
        while p > 0:
            ...
            # 找出 子 父 节点中 的最小节点
            if parent > 0 and self.compare_func(self.heap_list[parent]) < self.compare_func(self.heap_list[p]):
                min_index = parent
    ...
    def __up_to_down_min_heapify(self, p):
        ...
        while p < self.length:
            ...
            # 找出 当前节点 左子节点 和 右子节点  中的最小节点
            if left_child < self.length and self.compare_func(self.heap_list[left_child]) < self.compare_func(self.heap_list[min_index]):
                min_index = left_child

            if right_child < self.length and self.compare_func(self.heap_list[right_child]) < self.compare_func(self.heap_list[min_index]):
                min_index = right_child
...
if __name__ == '__main__':
    ...
    small_heap = HeapTuple([(4, 'a'), (3, 'b'), (3, 'c'), (1, 'd'), (2, 'e')], compare_func=lambda x: x[0], flag='small')
    print(small_heap)

    print('len(small_heap): ', len(small_heap))

    print('pop: ', small_heap.pop())
    print('len(small_heap): ', len(small_heap))
    print(small_heap)

    print(small_heap.sort())
